...
public class BlackJack extends CardGame{
    ...
    HashMap<Integer, String> possibleActionsWithoutSplit = new HashMap<Integer, String>(){{
        put(1,"Hit");
        put(2,"Stand");
        put(3,"Double Up");
    }};
    ...
    public BlackJack(DeckManager deckManager, ArrayList<Player> allPlayers, PlayerManager playerManager){
    ...
    private void playOnePlayerTurn(BlackJackPlayer player){
        while((player.isAbleToPlay() && player.wantsToPlay())){
            ...
            switch(promptPlayerAction(player)){
                case 1: hitPlayer(player);
                        break;
                case 2: standPlayer(player);
                        break;
                case 3: doubleUpPlayer(player);
                        break;
                case 4: if(splittable(player)){
                            splitPlayer(player);
                        }
                        else{
                            System.out.println("That option is not available for you right now");
                        }
                        break;
                default: break;
            }
    ...
    // Split the player's hand and adds the card as a new hand for the player
    private void splitPlayer(BlackJackPlayer player){
        if(splittable(player)){
            System.out.println("Your hand will now be split, please note that the new hand will be added to your hands\nand then hand you are currently playing with is noted. You will play with all hands\nuntil you stand them or their value becomes >= 21");
            Hand currentHand = player.getCurrentHand();
            Card splitCard = currentHand.getCards().get(1);
            Hand newHand = new Hand(new ArrayList<Card>());
            newHand.addCards(new Card[]{splitCard, drawCard()});
            player.removeCardFromCurrentHand(splitCard);
            drawCard(player);
            player.addHand(newHand);
            player.updateHandToBet();
        }
    }

    // Checks if the player can split their hand (has a pair of cards with the same rank)
    private boolean splittable(Player player){
        if(player.getCurrentHand().getNumCards() != 2){
            return false;
        }
        return player.getCurrentHand().getCards().get(0).getRank().equals(player.getCurrentHand().getCards().get(1).getRank());
    }
        ...
        do{
            try{
                ...
                if(splittable(player)){
                    ...
                    playerOptions = possibleActionsWithoutSplit;
                }
